...
def mel_spectrogram(y, n_fft, num_mels, sampling_rate, hop_size, win_size, fmin, fmax, center=False, mode='normal'):
    if torch.min(y) < -1.:
    ...
    y = torch.nn.functional.pad(y.unsqueeze(1), ((n_fft-hop_size)//2, (n_fft-hop_size)//2), mode='reflect')
    y = y.squeeze(1)
...
class VocDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.fine_tuning:
            ...
            voc_feats = vocoder_feature(audio, self.n_fft, self.sampling_rate, self.hop_size)
        ...
        return (voc_feats.squeeze(), audio.squeeze(0), filename, mel_loss.squeeze())
